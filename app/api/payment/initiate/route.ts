...
/** Maps checkout snapshot to WebX session3ds `customer` shape (official PHP / Postman samples). */
function webxCustomerFromCheckoutSnapshot(checkout: unknown, email: string): WebXCustomerProfile {
  const fallbackLocal = (email.split("@")[0] || "customer").slice(0, 48);
  const base: WebXCustomerProfile = {
    firstName: "Guest",
    lastName: fallbackLocal || "-",
  ...
  if (!checkout || typeof checkout !== "object") return base;
  const o = checkout as Record<string, unknown>;
  const sameAsDelivery = o.sameAsDelivery === true;
  const delivery = (o.delivery && typeof o.delivery === "object" ? o.delivery : {}) as Record<string, unknown>;
  const billing = (o.billing && typeof o.billing === "object" ? o.billing : {}) as Record<string, unknown>;

  const firstName = String(sameAsDelivery ? delivery.shippingFirstName : billing.billingFirstName ?? "").trim();
  const lastName = String(sameAsDelivery ? delivery.shippingLastName : billing.billingLastName ?? "").trim();
  const phone = String(sameAsDelivery ? delivery.shippingPhone : billing.billingPhone ?? "").trim();
  const addr = String(sameAsDelivery ? delivery.shippingAddressLine : billing.billingAddressLine ?? "").trim();
  const apt = String(sameAsDelivery ? delivery.shippingApartment : billing.billingApartment ?? "").trim();
  const city = String(sameAsDelivery ? delivery.shippingCity : billing.billingCity ?? "").trim();
  const postal = String(sameAsDelivery ? delivery.shippingPostalCode : billing.billingPostalCode ?? "").trim();

  const addressLineOne = [addr, apt].filter(Boolean).join(", ").trim();
  if (!firstName && !lastName && !phone && !addressLineOne && !city && !postal) return base;

  return {
    firstName: firstName || base.firstName,
    lastName: lastName || base.lastName,
    contactNumber: phone || base.contactNumber,
    addressLineOne: addressLineOne || base.addressLineOne,
    city: city || base.city,
    postalCode: postal || base.postalCode,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const customerProfile = webxCustomerFromCheckoutSnapshot(checkoutSnapshot, email);
