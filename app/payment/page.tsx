...
type InitiateResponse = {
  ...
  /** WebX ACS step: HTML document/fragment when no redirect URL (see `type: "3ds"` responses). */
  html3ds_html?: string | null;
  data?: unknown;
  error?: string;
  hint?: string;
  requestUrl?: string;
...
function extractHtml3dsHtml(data: unknown): string {
  if (!data || typeof data !== "object") return "";
  const obj = data as Record<string, unknown>;
  const raw = obj.html3ds ?? obj.html3DS;
  if (typeof raw === "string" && raw.trim()) return raw.trim();
  if (obj.data && typeof obj.data === "object") {
    return extractHtml3dsHtml(obj.data);
  }
  return "";
}
    ...
    generateSessionRef.current(
      async (sessionId: string) => {
        try {
          ...
          const html3dsHtml =
            (typeof json.html3ds_html === "string" && json.html3ds_html.trim()) ||
            extractHtml3dsHtml(json.data);

          if (!html3dsUrl && !html3dsHtml) {
            throw new Error("WebX did not return 3DS data (no redirect URL or bank HTML).");
          }

          writeWebxCheckoutSession({
            webx_order_id: json.order_id,
            html3ds_url: html3dsUrl || undefined,
          ...
          if (html3dsUrl) {
            ...
            const blob = new Blob([html3dsHtml], { type: "text/html;charset=utf-8" });
