...
export function getWebXApiConfig(): WebXApiConfig {
  const baseUrl = (process.env.WEBX_BASE_URL?.trim() || "").replace(/\/$/, "");
  ...
  const appBaseUrl = (process.env.WEBX_APP_BASE_URL?.trim() || "").replace(/\/$/, "");
  const testMode = process.env.WEBX_TEST_MODE === "true" || process.env.WEBX_TEST_MODE === "1";
  const jwtLoginUrl = (process.env.WEBX_JWT_LOGIN_URL?.trim() || "").replace(/\/$/, "");
  const initiateUrl = (process.env.WEBX_INITIATE_URL?.trim() || "").replace(/\/$/, "");
...
export function assertWebXApiConfigured(): void {
  ...
  if (!c.baseUrl || !c.username || !c.password || !c.merchantId) {
    throw new Error("[webxApi] Missing WEBX_BASE_URL, WEBX_API_USERNAME, WEBX_API_PASSWORD, or WEBX_MERCHANT_ID.");
  }
  if (!c.appBaseUrl) throw new Error("[webxApi] Missing WEBX_APP_BASE_URL.");
  if (!c.jwtLoginUrl) throw new Error("[webxApi] Missing WEBX_JWT_LOGIN_URL.");
  if (!c.initiateUrl) throw new Error("[webxApi] Missing WEBX_INITIATE_URL.");
}
...
export type WebXInitiateResult =
  | { ok: true; payment_url: string | null; raw: unknown }
...
export async function initiateWebXHostedPayment(input: InitiateHostedPaymentInput): Promise<WebXInitiateResult> {
  ...
  const body: Record<string, unknown> = {
    ...
    customer: { email: input.email, id: input.customerId },
  };
      ...
      response.status === 401 || response.status === 403
        ? String(
            rawObj?.message ??
              ...
              rawObj?.explanation ??
              "WebX rejected token for /cards/pay/session3ds. Verify WEBX_API_USERNAME/PASSWORD, WEBX_JWT_LOGIN_URL, WEBX_INITIATE_URL, and merchant access."
          )
        : undefined;
    return {
      ok: false,
  ...
  if (obj.error === true || String(obj.error).toLowerCase() === "true") {
    return {
      ok: false,
  ...
  const paymentUrl = pickString(obj, [
    "payment_url",
    "paymentUrl",
    "paymentPageUrl",
    "payment_page_url",
  ...
  const nestedUrl = nested
    ...
    : "";
  return { ok: true, payment_url: paymentUrl || nestedUrl || null, raw };
}
